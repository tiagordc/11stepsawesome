...
----
$ IP=$(minikube --profile 11steps ip)
...
----
$ docker images | grep 11steps
$ docker build -t 11stepsawesome/myboot:v1 .
----

Test that docker image
----
$ docker run -d -p 8080:8080 --name myboot 11stepsawesome/myboot:v1 
# OR
# docker run -d -p 8080:8080 --name myboot quay.io/burrsutter/myboot:v1
# wait for container to come up <1>
$ curl $(minikube -p 11steps ip):8080 <2>
...
----
$ curl $(minikube -p 11steps ip):$(kubectl get service/myboot -o jsonpath="{.spec.ports[*].nodePort}")
----
...
----
$ docker build -t 11stepsawesome/myboot:v2 .
$ docker images | grep myboot
----

Rollout the update
----
$ kubectl set image deployment/myboot myboot=11stepsawesome/myboot:v2
OR
...
----
$ curl $(minikube -p 11steps ip):$(kubectl get service/myboot -o jsonpath="{.spec.ports[*].nodePort}")/sysresources
 Memory: 1324 Cores: 2
----

Note: you can use echo to see what this URL really looks like on your machine
----
$ echo $(minikube -p 11steps ip):$(kubectl get service/myboot -o jsonpath="{.spec.ports[*].nodePort}")/sysresources
...
----
$ curl $(minikube -p 11steps ip):$(kubectl get service/myboot -o jsonpath="{.spec.ports[*].nodePort}")/consume
----
